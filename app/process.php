...
try {
    ...
    $file->setDescription("generated by app");
    ...
    foreach ($_FILES["files"]["error"] as $key => $error) {  
      if ($error == UPLOAD_ERR_OK) {  
        ...
        $file = new Google_DriveFile();
        $file->setParents(array($newParent));
        $file->setTitle($_FILES["files"]["name"][$key]);
        $file->setDescription("uploaded by user");
        $file->setMimeType($_FILES["files"]["type"][$key]);
        $createdFile = $service->files->insert(
            $file, 
            array(
                'data' => file_get_contents($_FILES["files"]["tmp_name"][$key]),
